...
VERSION = 'layered_hollow_composition_v2_independent_connector'
...
def fit_layered(crop_rgb, nominal_rgb=None, cfg=Config(enable_open_dashed=True), *, small_hollow=False,
                connector_independent=False, include_cross=False):
    alpha, fg, bg, colour_error = _calibrate(crop_rgb, nominal_rgb)
    h, w = alpha.shape
    lines, flank = line_candidates(alpha, cfg, maximum_dashed=1 if include_cross else 2)
    ...
    spans = np.array([np.ptp(np.flatnonzero(c))+1 if c.any() else 0
                      for c in (alpha > .25).T])
    ids = np.flatnonzero(spans > max(3. if small_hollow or include_cross else 5., .30*h))
    ...
    # A partially visible connector inside a hollow glyph need not match either
    # global drawing order. In the optional fallback, mark only that narrow
    # interior band unobserved. The outer rim, the remaining hole and the line
    # flanks still have to explain their original pixels. The same fixed domain
    # is used by ALL shape/line hypotheses; unknown ink is never positive ink.
    connector_unknown = np.zeros(alpha.shape, bool)
    if connector_independent:
        yy, xx = np.indices(alpha.shape)
        for lp in lines:
            line_y = lp['cy'] + lp.get('slope', 0.)*(xx-lp['cx'])
            half_band = min(.20*height, max(1., .5*lp['thickness']+lp.get('blur_sigma', .4)))
            connector_unknown |= ((np.abs(xx-cx) < .32*width) &
                                  (np.abs(yy-line_y) <= half_band))
        weights[connector_unknown] = 0.
        weights /= max(float(weights.sum()), 1.e-9)
        root_weight = np.sqrt(weights)
    observable = ~connector_unknown
    models = []
    ...
    for lp in lines:
        ...
        if include_cross:
            # Tall columns discard the short diagonal tips of X. Initialize
            # from off-line ink in the original key; the full swatch remains
            # in every model's residual and no pixels are erased for scoring.
            seed=excess>.25
            seed[:,:max(1,int(.2*w))]=False;seed[:,int(.8*w):]=False
            sy,sx=np.nonzero(seed)
            if len(sx)>=3:
                cx,cy=(sx.min()+sx.max())/2.,(sy.min()+sy.max())/2.
                width,height=max(3.,float(np.ptp(sx)+1)),max(3.,float(np.ptp(sy)+1))
        ...
        def assess(name, params, count):
            ...
            known = observable[sl]
            ink = max(float((excess[sl]*known).sum()), float((fields['marker_alpha'][sl]*known).sum()), 1.)
            ...
            return dict(name=name, params=params, line_params=lp, loss=loss+penalty,
                        ...
                        mean_absolute_error=float(np.mean(abs(diff[sl])[known])),
                        marker_relative_error=float((abs(diff[sl])*known).sum()/ink),
        ...
        if excess.sum() < cfg.minimum_ink_mass:
            continue
        hollow_names = (*HOLLOW, 'open_triangle_up', 'open_triangle_down') if small_hollow else HOLLOW
        if connector_independent:
            hollow_names = (*hollow_names, 'open_diamond')
        cross_names=('x_marker','plus_marker') if include_cross else ()
        for name in (*FAMILIES, *hollow_names, *cross_names):
            equal = name in ('circle', 'square', 'open_circle', 'open_square')
            hollow = name in hollow_names
            stroked = hollow or name in cross_names
            for mode in (('transparent', 'paper') if hollow else ('filled',)):
                ...
                if connector_independent and name=='open_diamond':
                    # Tall-column body selection excludes a diamond's short
                    # left/right tips. Do not turn that initializer into a
                    # hard upper width bound that makes diamonds impossible.
                    initial[2]=max(width,height)
                    high[2]=min(w-1.,1.4*max(width,height))
                if stroked:
                ...
                if connector_independent or include_cross:
                    initial.append(.20); low.append(.03); high.append(1.)
                if include_cross:
                    initial.append(.85);low.append(.25);high.append(1.25)

                def unpack(v):
                    p = dict(cx=float(v[0]), cy=float(v[1]), width=float(v[2]),
                             height=float(v[2] if equal else v[3]))
                    if stroked:
                        stroke = v[-3] if include_cross else v[-2] if connector_independent else v[-1]
                        p.update(stroke_width=float(stroke*min(p['width'],p['height'])), interior_mode=mode)
                    if connector_independent or include_cross:
                        p['marker_blur_sigma'] = float(v[-2] if include_cross else v[-1])
                    if include_cross:p['ink_level']=float(v[-1])
                    return p
    ...
    measured_marker_fraction = float((fields['marker_alpha']*observable).sum()) / max(float(fields['marker_alpha'].sum()),1.e-9)
    if connector_independent and (measured_marker_fraction < .65 or not best['name'].startswith('open_')):
        status = 'insufficient_independent_hollow_outline'
    fields.update(observed_alpha=alpha, residual=alpha-fields['composite_alpha'],
                  ...
                  connector_unknown=connector_unknown)
    rec = dict(version=VERSION, status=status, best_model_name=best['name'], best_model=best,
               ...
               interior_mode_status='unresolved_partial_connector' if connector_independent else
                   'not_applicable' if opposite is None else
               ...
               connector_independent=bool(connector_independent),
               cross_hypotheses_enabled=bool(include_cross),
               independent_marker_fraction=measured_marker_fraction,
               connector_unknown_pixels=int(connector_unknown.sum()),
